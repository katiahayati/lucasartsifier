...
| 5 | **rm380 → rm370** (Lady Celeste's spring, one visit) | sacredWater 40 | `[W]` drawn only inside; ~~poured into the old lamp at rm580~~ CORRECTED 2026-08-05: poured onto the SPELL PAGE from inventory (KqInv verb 24, `global161 \|= $0002`) — nothing pours at rm580 |
| 5b | **rm550 → rm580 / rm560 → rm580** (the Druid camp; the cage) | **nothing once flag 74 (befriended) is set; else huntersLamp 19 ∧ rain-ready (`global161 == 15`)** | `[U]` 2026-08-05: "same as the teacup — we need it to have water and ready to cast the make rain spell." The cage sorter is `(and (has: 19) (== global161 15))` (rm580.sc:1181) — lamp alone burns. All four readiness bits are established off-isle or from inventory (rm540 $0001, water-on-page $0002, baby's tears $0004, the cast $0008 at `==7`); NOTHING inside 580 sets any bit, and the first-visit capture gives no window. The teacup's ENTRANCE-half pattern, real instance. **DERIVED + PLACED 2026-08-06** as `(or (proc913_0 74) (and (has: 19) (== global161 15)))`. The 74 waiver covers the WHOLE demand — `[U]` 2026-08-06 on v24 in play: "we should let you revisit the camp without the lamp once there's no trap there" — and the derivation agrees twice over: `makeRain` RESETS global161 (rm580.sc:1007) as it sets 74 (an unconditional `==15` would wall every winner), and both capture armings (rm580:149/152) demand ¬74, so with the latch up there is no death left to guard. Derived from the surviving arm's own writes (reset + one-way latch), not named |
| 5c | **the isle LANDING itself, when the shore ambush is armed** (`flag 25 ∧ ¬14`; ¬74 is implied — 74 ⇒ 14) | same as 5b | `[W]` leave-and-return-unready dies ON ARRIVAL in stock: rm550's shore capture (`captured` → direct `newRoom: 580`) reaches the same cage sorter with no controllable moment after landing — the last crossing that can comply is boarding for the isle. Stage-conditioned like the cliff (demand only when the ambush state holds). **DERIVED + PLACED 2026-08-06 (v24)**: `(or (not (and (not (proc913_0 14)) (proc913_0 25))) <the 5b demand>)` on every crossing into rm550 — the magic-map click (`pullOutMapScr`, one travel-dispatch edit discriminated by `tpRoom 550` covers all four beach launches) plus rm560's west edge and rm580's return (edge-exit closes). The stage is `captured`'s own entry requirement, inherited from `waitForCapture`'s init arming through the machine-method armer chain. NOT play-tested |
