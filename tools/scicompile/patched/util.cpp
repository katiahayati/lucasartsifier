// PATCHED COPY (build-time) of util.cpp -- vendor original is UNMODIFIED.
// scicompile: ScriptId::GetFullPath uses '/' so Linux ifstream opens work (language
// detection, header loading). See BUILD_NOTES.md.
/***************************************************************************
...
std::string ScriptId::GetFullPath() const
{
    ...
    fullPath += "/";   // scicompile: POSIX separator (was "\\"), so ifstream opens succeed
...
void ScriptId::_Init(PCTSTR pszFullFileName, WORD wScriptNum)
{
    ...
    if (pszFullFileName)
    {
        ...
        // scicompile: split on the last path separator of EITHER kind. The
        // original splits only on '\\', but our GameFolderHelper builds POSIX
        // '/'-separated paths (GetScriptFileName), so a backslash-only search
        // finds nothing (-1) and mangles folder/filename. That in turn breaks
        // GetFullPath()/GetTitle() and the class browser's ReLoadFromSources
        // (which opens sources via GetFullPath). Use whichever separator occurs
        // last so both Windows and Linux paths parse correctly.
        int iIndexBS = str.ReverseFind('\\');
        int iIndexFS = str.ReverseFind('/');
        int iIndex = (iIndexFS > iIndexBS) ? iIndexFS : iIndexBS;
        if (iIndex < 0)
        {
            _strFolder = "";
            _strFileName = str;
        ...
        {
            _strFolder = str.Left(iIndex);
            _strFileName = str.Right(str.GetLength() - iIndex - 1);
        }
...
ScopedFile::ScopedFile(const std::string &filename, DWORD desiredAccess, DWORD shareMode, DWORD creationDisposition) : filename(filename)
{
    ...
    // scicompile: do NOT throw on a missing file. A headless compile of a
    // source-only project legitimately lacks compiled resources (resource.map,
    // vocab.*); callers already check hFile != INVALID_HANDLE_VALUE. Throwing here
    // aborts the whole run instead of degrading to "no compiled resources".
}
