// PATCHED COPY (build-time) of Stream.cpp -- vendor original is UNMODIFIED.
// scicompile: streamOwner(HANDLE) degrades to empty on INVALID handle (no throw).
/***************************************************************************
...
namespace sci
{
    ...
    {
        // scicompile: a missing/absent resource yields an INVALID handle. Degrade
        // to an empty stream rather than throwing (source-only projects have no
        // compiled resource.map / vocab files).
