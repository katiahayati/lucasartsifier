// PATCHED COPY (build-time) of Vocab99x.cpp -- vendor original is UNMODIFIED.
// scicompile: mechanical MSVC->GCC fixes only (rvalue istream& bindings; by-value proxy iterator). See BUILD_NOTES.md.
/***************************************************************************
...
bool KernelTable::Load(const GameFolderHelper &helper)
{
    ...
    if (blob)
    {
        { sci::istream __rs = blob->GetReadStream(); fRet = _Create(__rs, true); }
