    ...
    while (Q1 == *stream)
    {
        ...
        if (ch == Q2)
        {
            ...
            if (*stream == '+')
            {
                ...
                ++stream;
                _CommentPolicy::template EatWhitespaceAndComments<_TContext, _It>(nullptr, stream);
                // ... and continue...
            }
            else
            {
...
public:
    ...
    MatchResult Match(_TContext *pContext, _It &stream) const
    {
        ...
        if (!_fLiteral)
        {
            _CommentPolicy::template EatWhitespaceAndComments<_TContext, _It>(pContext, stream);
        }
        _It streamSave(stream);
#ifdef PARSE_DEBUG
        string text;
    ...
    template<typename _TCtxShadow, typename _ItShadowC, char Q1, char Q2>
