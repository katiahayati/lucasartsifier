...
|---|---|---|---|---|
| R1 | ✅ **VERIFIED 2026-08-05** (user: "the castle is verified") | hold + resume both confirmed | — | — |
| R2 | ✅ **VERIFIED 2026-08-05** (castle) | with-letter path = stock | — | — |
| R3 | ✅ **VERIFIED 2026-08-05** (castle) | treasures-first trace survives — finding #3's softlock is CLOSED in play | — | — |
| R4 | ✅ **VERIFIED 2026-08-05** (user: "the realm is verified") | cup conjunct at Charon confirmed | — | — |
| R5 | ✅ **VERIFIED 2026-08-05** (realm; on the clean install) | interior stock, no hang past Charon | — | — |
...
| R8 | ✅ **VERIFIED 2026-08-05** (user: "everything but the mists is tested") | cliff ALLOW + down-climb | — | — |
...
conspicuous. Closing it would mean shipping audio resource patches keyed like the talkers'
(a new resource type for the emitter) — "we should record a voice saying 'Just kidding' and
'not yet' lol". Parked as polish, not a defect.
...
|---|---|---|---|
| 2026-08-05 | Main | **FINDING #18 (real, user play, FIXED in v23): the mint/peppermint retraction CLOBBERS the joke — "neither can be dismissed, a huge mess"** (mint on self). Root cause: LAST IN TEXT IS NOT LAST ON SCREEN — the game's `(global91 say: …)` is the ASYNC Messager (queues a talker box), while our retraction is a MODAL Print, so the sibling statement opened on top of the still-queued joke and both fought for input claiming. LSL2 never showed it because its clauses speak through synchronous Print — sequential modals queue fine. Fix: when the surviving clause says through a messager whose CALLER slot is free, the retraction rides the say's completion — the caller argument points at an injected cue-object (`sgRetract23/31`, `(use …)` derived via `_class_owner_title`: KQ6 defines `Script` in System.sc, and a hardcoded `(use Script)` was itself a compile failure). Appended form stays as the fallback = LSL2's play-validated path, byte-identical. v23 = v22 + new 0.SCR/0.HEP only. Pinned GREEN in test_sci11_patch. | ✅ fixed in v23, PLAY-VERIFIED 2026-08-05 (user: "mints refusal works") |
| 2026-08-05 | rm580 | **FINDING #17 (real, user play, PINNED RED): the mists demand is the teacup's ENTRANCE half — lamp ∧ rain-ready, not lamp alone.** "Same as the teacup in the realm: we need it to have water and ready to cast the make rain spell." The cage sorter is `(and (has: 19) (== global161 15))` (rm580.sc:1181); the four readiness bits ($0001 rm540, $0002 water-on-page, $0004 baby's tears, $0008 the cast at ==7) are ALL established off-isle or from inventory, and rm580 only RESETS 161 — the item-half-only premise ("established inside") is false. WORSE than the teacup in one way: the revisit (25∧¬14∧¬74) shore capture carries you to the same sorter with no controllable moment after landing, so leave-and-return-unready dies on arrival IN STOCK — the last complying crossing is the landing itself. Both halves pinned RED in test_toll; guard oracle rows 5b/5c carry the ruling. | RED ×2; engine cure = carry the sorter's register literals (presentability-checked) + a stage-conditioned landing guard. ✅ CLOSED 2026-08-06 (v24): both REDs promoted; demand placed as `(and (has:19) (or (proc913_0 74) (== global161 15)))` at rm550/rm560→580 (the ¬74 waiver because makeRain resets 161 — an unconditional ==15 would wall winners) + the stage-conditioned landing on every crossing into rm550 incl. the magic-map click (travel-dispatch edit in pullOutMapScr, tpRoom-550-discriminated). RETEST: R6/R7 now = (a) first visit unready → landing ALLOWED (stage off), walk north refused (sgTurnBack demands lamp ∧ ready); (b) prepare fully (`vv g 161` reads 15) → capture → rain → befriended; (c) post-rain revisit: NOTHING demanded — lampless walk north ALLOWED (USER RULING 2026-08-06 on v24: "let you revisit the camp without the lamp once there's no trap there"; the 74 latch waives the whole demand — v25); (d) leave-unready-then-return → refused at the MAP CLICK "Not yet!", not burned at the cage. (a)/(b)/(d) + W1 PLAY-VERIFIED 2026-08-06 on v24; (c) re-test on v25 |
