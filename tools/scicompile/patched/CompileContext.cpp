// PATCHED COPY (build-time) of CompileContext.cpp -- vendor original is UNMODIFIED.
// scicompile: mechanical MSVC->GCC fixes only (no logic change). See BUILD_NOTES.md.
/***************************************************************************
...
void CompileContext::_LoadSCO(const std::string &name, bool fErrorIfNotFound)
{
    ...
    if (hFile != INVALID_HANDLE_VALUE)
    {
        ...
        CSCOFile scoFile;
        sci::istream __rdr = streamOwner.getReader();
        if (scoFile.Load(__rdr, _tables.Selectors()))
        {
...
bool CompileContext::LookupWord(const string &word, WORD &wWordGroup)
{
    ...
    // Source-only projects have no compiled parser vocabulary (vocab.000);
    // guard the null so Said words simply don't resolve instead of crashing.
    if (!_tables.Vocab()) { wWordGroup = 0; return false; }
