// PATCHED COPY (build-time) of SyntaxParser.cpp -- vendor original is UNMODIFIED.
// scicompile: mechanical MSVC->GCC fix only (rvalue const_iterator& binding). See BUILD_NOTES.md.
/***************************************************************************
...
{
    bool fRet = false;
    CCrystalScriptStream::const_iterator __sciStart = stream.begin();
    if (script.Language() == LangSyntaxStudio)
    {
        if (script.IsHeader())
        {
            fRet = g_studio.ParseHeader(script, __sciStart, preProcessorDefines, pLog, fParseComments);
        }
        else
        {
            if (pContext)
            {
                ...
                fRet = g_studio.Parse(script, __sciStart, preProcessorDefines, *pContext);
            }
            else
            {
                // Or maybe someone either wants error logs:
                fRet = g_studio.Parse(script, __sciStart, preProcessorDefines, pLog, addCommentsToOM, fParseComments);
            }
        }
    }
    else if (script.Language() == LangSyntaxSCI)
    {
        if (script.IsHeader())
        {
            fRet = g_sci.ParseHeader(script, __sciStart, preProcessorDefines, pLog, fParseComments);
        }
        else
        {
            if (pContext)
            {
                // Someone is doing a partial compile (e.g. tooltips) and supply their own context.
                fRet = g_sci.Parse(script, __sciStart, preProcessorDefines, *pContext);
            }
            else
            {
                // Or maybe someone either wants error logs:
                fRet = g_sci.Parse(script, __sciStart, preProcessorDefines, pLog, addCommentsToOM, fParseComments);
            }
        }
