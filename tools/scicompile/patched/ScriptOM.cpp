// PATCHED COPY (build-time) of ScriptOM.cpp -- vendor original is UNMODIFIED.
...
std::string Comment::GetSanitizedText() const
{
	...
	if ((_innerName[0] == '/') && (_innerName[1] == '/'))
    {
        ...
        { std::string __t = _innerName.substr(2); return trim(trim(__t, '\n'), ' '); }
    }
    else if (_innerName[0] == ';')
    {
        // Remove the endline and any spaces
        { std::string __t = _innerName.substr(1); return trim(trim(__t, '\n'), ' '); }
    }
