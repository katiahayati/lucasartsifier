// PATCHED COPY (build-time) of ClassBrowser.cpp -- vendor original is UNMODIFIED.
// scicompile: mechanical MSVC->GCC fixes only (typename; by-value proxy iterator). See BUILD_NOTES.md.
/***************************************************************************
...
typename T::pointer lookup_ptr(const std::unordered_map<T2, T> &map, const T2 &str)
{
	...
	typename std::unordered_map<T2, T>::const_iterator nodeIt = map.find(str);
    if (nodeIt != map.end())
    {
...
void SCIClassBrowser::ReLoadFromCompiled(ITaskStatus &task)
{
    ...
    for (auto scriptBlob : *resourceContainer)  // proxy iterator yields unique_ptr by value
    {
