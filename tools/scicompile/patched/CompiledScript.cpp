// PATCHED COPY (build-time) of CompiledScript.cpp -- vendor original is UNMODIFIED.
// scicompile: mechanical MSVC->GCC fix only (rvalue istream& bindings). See BUILD_NOTES.md.
/***************************************************************************
...
bool CompiledScript::Load(const GameFolderHelper &helper, SCIVersion version, int iScriptNumber)
{
    ...
    if (scriptResource)
    {
        { sci::istream __rs = scriptResource->GetReadStream(); Load(helper, version, iScriptNumber, __rs); }
        return true;
    }
    return false;
}
    ...
    if (!filename.empty())
    {
        ...
        if (hFile != INVALID_HANDLE_VALUE)
        {
            try
            {
                ...
                if (scoFile.Load(__rdr, _selectors))
                {
