// PATCHED COPY (build-time) of SCO.cpp -- vendor original is UNMODIFIED.
// scicompile: mechanical MSVC->GCC fix only (typename on dependent iterator; rvalue istream& binding). See BUILD_NOTES.md.
/***************************************************************************
...
{
    typename vector<_T>::const_iterator position = match_name_dot(container.begin(), container.end(), name);
