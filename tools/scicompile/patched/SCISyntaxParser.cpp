...
// SIERRA_SOFTLOCK PATCH: '#' is also legal as a CONTINUATION character. KQ6's own vocab.997
// names selector 879 'dungeon#', and the decompiled rm710.sc both declares and reads that
// property; the stock lexer stopped at '#', producing "Expected an expression" on the game's
// own source. Prefix '#' (the selector-literal token) is unaffected -- this only extends a
// token that has already begun.
template<typename _It>
