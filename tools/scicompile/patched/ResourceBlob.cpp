// PATCHED COPY (build-time) of ResourceBlob.cpp -- vendor original is UNMODIFIED.
// scicompile: mechanical MSVC->GCC fixes only (rvalue istream& binding; atomic direct-init). See BUILD_NOTES.md.
/***************************************************************************
...
HRESULT ResourceBlob::CreateFromHandle(PCTSTR pszName, HANDLE hFile, int iPackageHint, SCIVersion version, ResourceSaveLocation saveLocation)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        { sci::istream __rs = streamOwner.getReader(); _DecompressFromBits(__rs, false); }
