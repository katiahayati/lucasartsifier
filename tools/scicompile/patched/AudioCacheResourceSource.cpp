// PATCHED COPY (build-time) of AudioCacheResourceSource.cpp -- vendor original is UNMODIFIED.
// scicompile: mechanical MSVC->GCC fixes only (by-value proxy iterators; rvalue istream&). See BUILD_NOTES.md.
/***************************************************************************
...
void AudioCacheResourceSource::MaybeAddNegative(ResourceEntity &resource)
{
    try
    {
        ...
        if (PathFileExists(fullPath.c_str()))
        {
            ...
            { sci::istream __rs = owner.getReader(); AudioComponentFromWaveFile(__rs, negative->Audio, &negative->Settings); }
