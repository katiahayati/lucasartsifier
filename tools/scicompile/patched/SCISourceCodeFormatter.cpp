// PATCHED COPY (build-time) of SCISourceCodeFormatter.cpp -- vendor original is UNMODIFIED.
// scicompile: mechanical MSVC->GCC fix only (bool false -> nullptr for pointer arg). See BUILD_NOTES.md.
/***************************************************************************
...
public:
    ...
    {
        uint16_t result;
        return condExp.Evaluate(simpleDefineLookup, result, nullptr) && (result == 1);
    }
